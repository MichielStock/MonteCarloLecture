...
md"""
## 
We can turn these reactions into a simple system of ordinary differential equations:
"""
...
The general Monte-Carlo method is given by the following recipe:

1. randomly sample a parameter value $\theta \sim P(\theta)$;
2. perform a simulation (called a **throw**) to generate an $x$ using the model (i.e. $P(X\mid \theta)$);
3. repeat steps 1 & 2 $n$ times to obtain a statistically representative sample;
4. combine all simulation results for statistical analysis.
"""

# ╔═╡ fbee599e-5784-4c77-bcc9-c3996536e4df
md"## Implementation of the Monte-Carlo method"
...
md"""
## Scenario 1: uncertainty on $\gamma$
...
md"""
## Scenario 2: uncertainty on $\gamma$ and $k_i$
...
md"""##
We can model this trade-off between $\gamma$ and $k_i$ using a **multivariate normal distribution** with a correlation coefficient $\rho$.
"""
...
html"<button onclick='present()'>leave presentation</button>"
...
function virus_model!(du, u, (γ, kᵢ, kᵥ), t)
	Nᵢ, Nᵥ = u  # unpack variables
	du[1] = Ṅᵢ = - kᵢ * Nᵢ
	du[2] = Ṅᵥ = - kᵥ * Nᵥ + γ * Nᵢ
end

# ╔═╡ 0293d9be-4d85-4829-a1b7-6f341c9f694b
prob_ode = ODEProblem(virus_model!, [Nᵢ₀, Nᵥ₀], (0.0, 8.0), (γ, kᵢ, kᵥ))

# ╔═╡ 548c2643-3742-4b50-8aa6-9988e8c5356e
md"Some plotting functions."
...
pmc1 = plotMC(throws1; title="MC scenario 1\n(sample from Pγ)")
...
Σ = [σ_γ^2 ρ*σ_γ*σ_ki; ρ*σ_γ*σ_ki σ_ki^2]

# ╔═╡ 83ab2f8d-98f8-4307-b1a0-c3bc4c36a682
Pγ_kᵢ_tradeoff = MultivariateNormal([γ_star, kᵢ_star], Σ);
...
[[deps.Catalyst]]
deps = ["AbstractAlgebra", "DataStructures", "DiffEqBase", "DiffEqJump", "DocStringExtensions", "Graphs", "LaTeXStrings", "Latexify", "MacroTools", "ModelingToolkit", "Parameters", "Reexport", "Requires", "SparseArrays", "Symbolics"]
git-tree-sha1 = "6374f14912732479aca4e61c42cf1cc79c2af25c"
...
git-tree-sha1 = "a8fdb7951e7bd6bc3eec387238bb74b19cbc4743"
...
git-tree-sha1 = "3c4daf04e71c13ebb0d4d06f7437f617d878854e"
